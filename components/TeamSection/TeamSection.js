...
const TeamSection = (props) => {
  ...
  return (
    <section className={'' + props.hclass}>
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-6 col-12">
              <SectionTitle
                subtitle={'The Team at Shamba Records'}
                title={'Passionate Minds, Real Impact.'}
              />
            </div>
          </div>
        </div>
          ...
          <Slider {...settings}>
            {TeamData.map((team, index) => (
              <div className="team-card" key={team.id}>
                {/* Fixed-size circular wrapper; Image uses fill inside */}
                <div className="image">
                  <Image
                    src={team.image}
                  ...
                  <div className="icon">
                    <ul className="flex gap-3">
                      ...
                      <li>
                        <Link onClick={ClickHandler} href={`mailto:${team.email}`}>
      ...
      {/* Scoped styles to lock avatar size and keep it circular */}
      <style jsx>{`
        .team-card .image {
          position: relative;
          width: 200px;
        ...
        @media (max-width: 1399px) {
          .team-card .image { width: 180px; height: 180px; }
        }
        @media (max-width: 1199px) {
          .team-card .image { width: 170px; height: 170px; }
        }
        @media (max-width: 991px) {
          .team-card .image { width: 200px; height: 200px; }
        }
        @media (max-width: 575px) {
          .team-card .image { width: 220px; height: 220px; }
        }
      `}</style>
    </section>
  );
